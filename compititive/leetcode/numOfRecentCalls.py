...
class RecentCounter:
    ...
    def binarySearch(arr,s,e,val):
        ...
        mid = (s+e)//2
        if(arr[mid] >= val):
            return RecentCounter.binarySearch(arr,s,mid,val)
        else:
            return RecentCounter.binarySearch(arr,mid+1,e,val)
    '''
    def ping(self, t: int) -> int:
        ...
        if(self.start == -1):
            ...
            return 1
        n = len(self.requests)
        index = RecentCounter.binarySearch(self.requests,self.start,n-1,t-3000)
        self.start = index
        return n - index
        '''
        self.q.append(t)
